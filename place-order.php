<?php
  ...
  $paymentModeResult = getPaymentModes();
        ...
        <div class="col-md-8 offset-md-2">
            <div class="card">
                ...
                <div class="card-body pb-0">
                    <?php
                        if(isset($orderResult['status']) && $orderResult['status'] == 'false'){
                            ?>
                            <div class="alert alert-danger"><?=$orderResult['message'];?></div>
                            <?php
                        }
                    ?>
                    <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']);?>" method="post">
                        <div class="form-group">
                            ...
                            <textarea name="address" class="form-control" rows="8" placeholder="Address" required></textarea>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select name="paymentMode" id="paymentMode" class="form-control" required>
                                <option hidden>Choose Payment mode</option>
                                <?php
                                    if($paymentModeResult->num_rows > 0){
                                        while($rows = $paymentModeResult->fetch_assoc()){
                                            ?>
                                                <option value="<?=$rows['id'];?>"><?=$rows['payment_mode'];?></option>
                                            <?php
                                        }
                                    }
                                ?>
                        ...
                        <div class="form-group">
                            <label for="total Price">Total Amount</label>
                            <input name="totalPrice" id="totalPrice" type="text" class="form-control" placeholder="State" required value="" disabled>
                        </div>
...
<script>
    ...
    for(var i=0, len=localStorage.length; i<len; i++) {
        ...
        if(key.match(regexp)){
            ...
            products.push({id:id[1],qty:item.orderQuantity});
